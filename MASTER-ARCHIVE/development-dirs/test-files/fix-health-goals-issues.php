...
if ( $current_user->ID ) {
	...
	if ( ! empty( $old_goals ) && empty( $new_goals ) ) {
		...
		$default_goals = array( 'energy', 'sleep', 'stress' );
...
if ( file_exists( $config_file ) ) {
	try {
		...
		if ( isset( $config['goal_definitions'] ) ) {
			...
			$test_goals    = array( 'energy', 'strength', 'sleep' );
			$matched_goals = 0;
...
if ( class_exists( 'ENNU_Intentionality_Engine' ) && $current_user->ID ) {
	...
	if ( ! empty( $user_goals ) ) {
		try {
			...
			$goal_definitions    = file_exists( $health_goals_config ) ? require $health_goals_config : array();
...
if ( $current_user->ID && class_exists( 'ENNU_Health_Goals_Ajax' ) ) {
	try {
		...
		$_POST['health_goals'] = array( 'energy', 'sleep', 'strength' );
