...
class ENNU_Score_Presentation_Shortcode extends ENNU_Base_Shortcode {
	...
	private function get_life_score_data( $user_id ) {
		...
		// Fix: Use the correct meta key that matches what the dashboard saves
	...
	private function get_pillar_score_data( $user_id, $pillar_type ) {
		...
		// Fix: Get pillar score from the average pillar scores array
		$average_pillar_scores = get_user_meta( $user_id, 'ennu_average_pillar_scores', true );
		$average_pillar_scores = is_array( $average_pillar_scores ) ? $average_pillar_scores : array();
		
		// Get the score for this specific pillar
		$score = isset( $average_pillar_scores[ ucfirst( $pillar_key ) ] ) ? $average_pillar_scores[ ucfirst( $pillar_key ) ] : 0;
	...
	protected function get_template( $template_name, $variables = array() ) {
		...
		// Fix template path to look in the correct directory
