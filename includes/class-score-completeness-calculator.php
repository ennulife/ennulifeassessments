...
class ENNU_Score_Completeness_Calculator {
	...
	public function __construct( $user_id, $all_definitions ) {
		...
		// REMOVED: // REMOVED DEBUG LOG: error_log( "ScoreCompletenessCalculator: Instantiated for user ID {$user_id}." );
	}

	public function calculate() {
		// REMOVED: // REMOVED DEBUG LOG: error_log( 'ScoreCompletenessCalculator: Starting calculation.' );
		$achieved_points = 0;
		...
		if ( $total_assessments > 0 ) {
			...
			// REMOVED: error_log( "ScoreCompletenessCalculator: User has completed {$completed_assessments}/{$total_assessments} assessments, awarding points." );
		}
		...
		if ( ! empty( $health_opt_data ) ) {
			...
			// REMOVED: error_log( 'ScoreCompletenessCalculator: Health optimization completed, awarding points.' );
		}
		...
		if ( get_user_meta( $this->user_id, 'ennu_global_health_goals', true ) ) {
			...
			// REMOVED: error_log( 'ScoreCompletenessCalculator: Health goals are set, awarding points.' );
		}

		$final_score = round( $achieved_points );
		// REMOVED: // REMOVED DEBUG LOG: error_log( "ScoreCompletenessCalculator: Final completeness score: {$final_score}" );
		return $final_score;
	}
}
