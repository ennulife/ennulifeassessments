...
class ENNU_Multi_Tenant_Manager {
    ...
    private function apply_tenant_branding() {
        ...
        if (!empty($config['logo_url'])) {
            add_filter('get_custom_logo', function() use ($config) {
        ...
        if (!empty($config['site_title'])) {
            add_filter('bloginfo', function($output, $show) use ($config) {
                if ($show === 'name') {
    ...
    public function set_tenant_config($key, $value, $tenant_id = null) {
        ...
        if ($existing) {
            $wpdb->update(
                ...
                array('tenant_id' => $tenant_id, 'config_key' => $key),
                array('%s'),
                array('%d', '%s')
            );
        } else {
    ...
    public function handle_create_tenant() {
        ...
        if ($result) {
            wp_send_json_success(array('message' => 'Tenant created successfully', 'tenant_id' => $wpdb->insert_id));
        } else {
