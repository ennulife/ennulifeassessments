...
class ENNU_Security_Audit_Logger {
    ...
    public function log_user_login($user_login, $user) {
        $this->log_event('user_login', "User {$user_login} logged in successfully", array(
            'user_id' => $user->ID,
    ...
    public function log_user_logout() {
        ...
        if ($user->ID) {
            $this->log_event('user_logout', "User {$user->user_login} logged out", array(
                'user_id' => $user->ID,
    ...
    public function log_failed_login($username) {
        $this->log_event('failed_login', "Failed login attempt for username: {$username}", array(
            'username' => $username,
    ...
    public function log_password_reset($user, $new_pass) {
        $this->log_event('password_reset', "Password reset for user: {$user->user_login}", array(
            'user_id' => $user->ID,
            'user_login' => $user->user_login,
            'user_email' => $user->user_email
        ));
    ...
    public function log_user_registration($user_id) {
        ...
        $this->log_event('user_registration', "New user registered: {$user->user_login}", array(
            'user_id' => $user_id,
            'user_login' => $user->user_login,
            'user_email' => $user->user_email
        ));
    ...
    public function log_role_change($user_id, $role, $old_roles) {
        $user = get_user_by('ID', $user_id);
        $this->log_event('user_role_changed', "User role changed for {$user->user_login}", array(
            'user_id' => $user_id,
            'user_login' => $user->user_login,
            'new_role' => $role,
            'old_roles' => $old_roles
    ...
    public function log_plugin_activation($plugin) {
        $this->log_event('plugin_activated', "Plugin activated: {$plugin}", array(
            'plugin' => $plugin
        ));
    }
    ...
    public function log_plugin_deactivation($plugin) {
        $this->log_event('plugin_deactivated', "Plugin deactivated: {$plugin}", array(
            'plugin' => $plugin
        ));
    }
    ...
    public function log_theme_switch($new_name, $new_theme, $old_theme) {
        $this->log_event('theme_switched', "Theme switched to: {$new_name}", array(
            'new_theme' => $new_name,
    ...
    public function log_option_update($option, $old_value, $value) {
        ...
        if (in_array($option, $monitored_options)) {
            $this->log_event('option_updated', "Option updated: {$option}", array(
                'option' => $option,
                'old_value' => $old_value,
                'new_value' => $value
            ));
    ...
    public function log_ip_blocked($ip, $reason) {
        $this->log_event('ip_blocked', "IP address blocked: {$ip}", array(
            'ip' => $ip,
            'reason' => $reason
        ));
    }
    ...
    public function log_rate_limit($ip, $limit_type) {
        $this->log_event('rate_limit_exceeded', "Rate limit exceeded for IP: {$ip}", array(
            'ip' => $ip,
    ...
    public function cleanup_old_logs() {
        ...
        if ($deleted !== false) {
            $this->log_event('log_cleanup', "Cleaned up {$deleted} old audit log entries", array(
