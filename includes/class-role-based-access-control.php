...
class ENNU_Role_Based_Access_Control {
    ...
    public function get_access_log($filters = array()) {
        ...
        if (!empty($filters)) {
            $access_log = array_filter($access_log, function($entry) use ($filters) {
    ...
    public function get_security_summary() {
        ...
        $summary = array(
            ...
            'denied_attempts' => count(array_filter($recent_log, function($entry) {
        ...
        $denials = array_filter($recent_log, function($entry) {
            return $entry['result'] === 'denied';
        });
