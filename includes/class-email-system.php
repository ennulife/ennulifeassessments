...
class ENNU_Life_Email_System {
    ...
    public function send_assessment_results($assessment_type, $results, $user_email) {
        ...
        $subject = str_replace('{assessment_type}', ucwords(str_replace('-', ' ', $assessment_type)), 
    ...
    private function send_admin_form_notification($form_type, $form_data, $submission_id) {
        $subject = str_replace('{form_type}', ucwords(str_replace('-', ' ', $form_type)), 
    ...
    private function send_email($to, $subject, $message, $template_type = '') {
        ...
        // Log email sending
        if ($sent) {
            error_log("ENNU Life: Email sent successfully to {$to} - Subject: {$subject}");
        } else {
            error_log("ENNU Life: Failed to send email to {$to} - Subject: {$subject}");
        }
